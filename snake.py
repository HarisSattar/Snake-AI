...
def index_to_pixel(part):
    x = part.x * BLOCK_W
    y = part.y * BLOCK_H
    pygame.draw.rect(screen, GREEN, (x, y, BLOCK_W, BLOCK_H))


def draw():
    screen.fill(BLACK)   
    apple = pygame.draw.rect(screen, RED, (apple_x * BLOCK_W, apple_y * BLOCK_H, BLOCK_W, BLOCK_H))
    deque(map(index_to_pixel, snake))
...
while True:
    ...
    for e in pygame.event.get():
        if e.type == QUIT:
            sys.exit(0)
        # elif e.type == KEYDOWN:
        #     if (e.key == K_UP or e.key == K_w)  and dirs != 0:
        #         dirs = 2
        #         break
        #     elif (e.key == K_DOWN or e.key == K_s) and dirs != 2:
        #         dirs = 0
        #         break                
        #     elif (e.key == K_LEFT or e.key == K_a) and dirs != 1:
        #         dirs = 3
        #         break                
        #     elif (e.key == K_RIGHT or e.key == K_d) and dirs != 3:
        #         dirs = 1
        #         break                
        #     if e.key == pygame.K_p:
        #         dirs = -1
